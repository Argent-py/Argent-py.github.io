// main.js - enhanced for cart (localStorage), quick view, live search, theme toggle, lazy load helpers
...
function formatToman(number) {
    ...
    // format with thousands separator (use Persian digits if desired)
...
// Delegation for qty change and remove
...
// افکت ورود کلمه به کلمه برای hero-title
document.addEventListener("DOMContentLoaded", () => {
  const spans = document.querySelectorAll(".hero-title.animate-blur span");
  spans.forEach((span, index) => {
    span.style.animationDelay = `${index * 0.25}s`; // هر کلمه ۰.۲۵ ثانیه تأخیر
  });
});
